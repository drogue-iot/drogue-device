...
impl LowerAccessMessage {
    pub fn emit<const N: usize>(&self, xmit: &mut Vec<u8, N>) -> Result<(), InsufficientBuffer> {
        match self {
            ...
            LowerAccessMessage::Segmented { szmic, seq_zero, seg_o, seg_n, segment_m } => {
                let mut header = [0;3];
                ...
                xmit.extend_from_slice(&header).map_err(|_|InsufficientBuffer)?;
                xmit.extend_from_slice(segment_m).map_err(|_|InsufficientBuffer)?;
                Ok(())
            }
        }
    }
